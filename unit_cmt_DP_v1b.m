% With parallelization; no economic wind curtailment
...
%     
%     f_sum_tmp = f_table_sum(:,id_tmp); % [x]x[1] = [14x1]
%     f_tmp = f_table(:,:,id_tmp); % [14 units]x[x] = [14x14]
%     u_tmp = u_table(:,:,id_tmp); % [14 units]x[x] = [14x14]
%     v_tmp = v_table(:,:,id_tmp); % [14 units]x[x] = [14x14]
%     
%     cost_base_vom_tmp = coal_pwr_tmp * coal_baseload; % [1x1]
...
%     
%     u_rep = repmat(u_tmp,1,1,coal_num); % [14 units]x[state]x[u]=[14]x[14]x[14]
%     u_flip = repmat(reshape(u_dispatch(:,:,t+1),coal_num,1,coal_num),1,coal_num);
%     d_coal_pwr = u_rep - u_flip; % [14x14x14]
%     d_coal_pctg = d_coal_pwr/coal_nameplate;
%     x = abs(d_coal_pctg);
%     y = (x-0.3)*6.5/0.7+1.5;
%     ramp_scale = ones(size(x));
%     ramp_scale(x>0.3) = y(x>0.3);
%     cost_ramp_tmp = squeeze(sum(abs(d_coal_pwr).*ramp_scale * coal_loadfollow)); % [14]x[14] = [state]x[u]
%     
%     J_tmp = repmat(J_star(:,t+1)',coal_num,1) + repmat(cost_fuel_tmp,1,coal_num) + cost_base_vom_tmp + cost_startup_tmp + cost_ramp_tmp; % [14]x[14] = [state]x[u]
...
%     id_col = id_opt';
%     id_row = 1:coal_num;
%     id_extract = sub2ind([coal_num, coal_num], id_row, id_col);
%     
%     opt_cost_startup(:,t) = cost_startup_tmp(id_extract);
%     opt_cost_base_vom(:,t) = cost_base_vom_tmp;
%     opt_cost_fuel(:,t) = cost_fuel_tmp;
%     opt_cost_ramp(:,t) = cost_ramp_tmp(id_extract);
% end
% toc;
